...
object LocationRepository {
    fun getDeviceId(context: Context): String = context.getSharedPreferences("tracker", Context.MODE_PRIVATE).getString("deviceId", null)?.trim().orEmpty()
    ...
    suspend fun fetchAndUpload(context: Context): String {
        ...
        val location = suspendCancellableCoroutine<Location> { cont ->
            client.getCurrentLocation(Priority.PRIORITY_BALANCED_POWER_ACCURACY, null).addOnSuccessListener { loc -> if (loc != null) cont.resume(loc) else cont.resumeWithException(IllegalStateException("Location unavailable")) }.addOnFailureListener { cont.resumeWithException(it) }
        }
        ...
        val data = hashMapOf<String, Any?>(
            ...
            "locationFetchedAt" to FieldValue.serverTimestamp()
        )
