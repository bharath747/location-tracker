    ...
    suspend fun fetchAndUpload(context: Context): String {
        ...
        val location = suspendCancellableCoroutine<Location> { cont ->
            client.getCurrentLocation(Priority.PRIORITY_BALANCED_POWER_ACCURACY, null)
                .addOnSuccessListener { loc -> if (loc != null) cont.resume(loc) else cont.resumeWithException(IllegalStateException("Location unavailable")) }
        ...
        val data = hashMapOf<String, Any?>(
            ...
            "accuracy" to location.accuracy, "batteryLevel" to battery,
            "isCharging" to charging, "lastSeen" to FieldValue.serverTimestamp()
        )
        val doc = FirebaseFirestore.getInstance().collection("devices").document(getDeviceId(context))
