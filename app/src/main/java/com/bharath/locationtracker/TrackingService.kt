        ...
        commandListener = ref.collection("commands")
            ...
            .addSnapshotListener { snapshots, _ ->
                snapshots?.documents?.forEach { command ->
                    executeCommand(command.id, command.getString("action")?.uppercase() ?: return@forEach)
                }
            }
    }
    ...
    private fun executeCommand(commandId: String, action: String) {
        ...
        when (action) {
            "RING" -> {
                ...
                ref.update("status", "EXECUTED", "executedAt", com.google.firebase.firestore.FieldValue.serverTimestamp())
            }
            "STOP_RING" -> {
            ...
            "FETCH_LOCATION" -> {
                CoroutineScope(Dispatchers.IO).launch {
                    runCatching { LocationRepository.fetchAndUpload(this@TrackingService) }
                        ...
                        .onFailure {
                            ref.update("status", "FAILED", "error", it.message ?: "Location fetch failed")
                        }
                }
            }
            else -> ref.update("status", "IGNORED")
        }
    }
