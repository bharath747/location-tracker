...
class MainActivity : ComponentActivity() {
            ...
            MaterialTheme {
                if (adminMode) {
                    ...
                    TrackerScreen(
                        ...
                        { startTracking() }, { stopTracking() }, { testRing() },
    ...
    private fun getTrackerDeviceId(): String {
        ...
        return p.getString("deviceId", null)
    ...
    private fun signInAndRegister() {
        ...
        if (a.currentUser != null) registerDevice()
        else a.signInAnonymously().addOnSuccessListener { registerDevice() }
    ...
    private fun registerDevice() {
        ...
        FirebaseFirestore.getInstance().collection("devices").document(deviceId).set(d)
    ...
    private fun restoreTrackingIfPossible() {
        ...
        if (granted) {
            try {
                ...
                status = "Could not restore tracking: ${e.message}"
            }
        } else {
            trackingActive = false
            getSharedPreferences("tracker", Context.MODE_PRIVATE).edit().putBoolean("trackingEnabled", false).apply()
            status = "Tracking paused: location permission required"
        }
    }

    private fun startTracking() {
        val g = ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED ||
            ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED
    ...
    private fun fetchAndUploadLocation() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            status = "Please grant location permission"; return
        }
        status = "Fetching location..."
        lifecycleScope.launch {
            try { locationText = LocationRepository.fetchAndUpload(this@MainActivity); status = "Location uploaded to Firebase" }
    ...
    private fun sendCommand(id: String, action: String) {
        ...
        ).addOnSuccessListener { adminStatus = "$action command created for $id" }
    ...
    private fun updateInterval(id: String, minutes: Int) {
        FirebaseFirestore.getInstance().collection("devices").document(id).collection("config").document("tracking")
            ...
            .addOnSuccessListener { adminStatus = "Interval updated to ${minutes.coerceAtLeast(15)} minutes" }
...
private fun TrackerScreen(status: String, location: String, deviceId: String, tracking: Boolean, alarm: String, listener: String, lastCommand: String, commandResult: String, onGrant: () -> Unit, onFetch: () -> Unit, onStart: () -> Unit, onStop: () -> Unit, onRing: () -> Unit, onStopAlarm: () -> Unit, onAdmin: () -> Unit) {
    LazyColumn(Modifier.fillMaxSize().padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item {
            ...
            Text("Background Tracking: ${if (tracking) "ACTIVE" else "STOPPED"}")
            Button(onClick = if (tracking) onStop else onStart, modifier = Modifier.fillMaxWidth()) { Text(if (tracking) "Stop Background Tracking" else "Start Background Tracking") }
            HorizontalDivider()
            Text("Remote Commands", style = MaterialTheme.typography.titleLarge)
            Text("Command Listener: $listener")
            Text("Last Command: $lastCommand")
            Text("Last Result: $commandResult")
            HorizontalDivider()
            Text("Remote Alarm", style = MaterialTheme.typography.titleLarge)
            Text("Alarm Status: $alarm")
            Button(onClick = onRing, modifier = Modifier.fillMaxWidth()) { Text("Test Ring") }
            OutlinedButton(onClick = onStopAlarm, modifier = Modifier.fillMaxWidth()) { Text("Stop Alarm") }
            HorizontalDivider()
            Button(onClick = onAdmin, modifier = Modifier.fillMaxWidth()) { Text("Open Admin Screen") }
        }
    }
}
    ...
    LazyColumn(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text("Location Tracker Admin", style = MaterialTheme.typography.headlineSmall)
                TextButton(onClick = onBack) { Text("Tracker") }
            }
            Text(status)
            Button(onClick = onRefresh, modifier = Modifier.fillMaxWidth()) { Text("Refresh Devices") }
            OutlinedTextField(value = interval, onValueChange = { interval = it.filter(Char::isDigit) }, label = { Text("Tracking interval minutes (min 15)") }, modifier = Modifier.fillMaxWidth())
        }
        items(devices) { d ->
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        OutlinedButton(onClick = { onStop(d.id) }) { Text("Stop Ring") }
                        OutlinedButton(onClick = { onInterval(d.id to (interval.toIntOrNull() ?: 15)) }) { Text("Set Interval") }
                    }
                }
            }
        }
